import { useEffect, useState } from "react";
import { useAuth } from "@/lib/clerk";

interface BunnyPlayerProps {
  videoId: string;
  courseId: string;
  title: string;
}

export function BunnyPlayer({ videoId, courseId, title }: BunnyPlayerProps) {
  const { getToken, isLoaded, isSignedIn } = useAuth();
  const [embedUrl, setEmbedUrl] = useState<string | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    async function fetchToken() {
      try {
        // Free videos — no auth needed, call edge function without token
        const headers: Record<string, string> = {
          "Content-Type": "application/json",
        };

        // Only attach Clerk token for paid videos
        if (courseId !== "free") {
          if (!isSignedIn) {
            setError("Please sign in to watch this video");
            setLoading(false);
            return;
          }
          const clerkToken = await getToken();
          headers["Authorization"] = `Bearer ${clerkToken}`;
        }

        const res = await fetch(
          `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/get-video-token`,
          {
            method: "POST",
            headers,
            body: JSON.stringify({ videoId, courseId }),
          }
        );

        if (!res.ok) {
          const err = await res.json();
          setError(err.error);
          return;
        }

        const { token, expires, libraryId } = await res.json();
        const url = `https://iframe.mediadelivery.net/embed/${libraryId}/${videoId}?token=${token}&expires=${expires}&autoplay=false`;
        setEmbedUrl(url);
      } catch (err) {
        setError("Failed to load video");
      } finally {
        setLoading(false);
      }
    }

    // Wait for Clerk to finish loading before doing anything
    if (isLoaded) {
      fetchToken();
    }
  }, [videoId, courseId, isLoaded, isSignedIn]);

  if (!isLoaded || loading) {
    return (
      <div className="aspect-video bg-gray-900 rounded-lg flex items-center justify-center">
        <div className="text-white text-sm animate-pulse">Loading video...</div>
      </div>
    );
  }

  if (error) {
    return (
      <div className="aspect-video bg-gray-900 rounded-lg flex items-center justify-center">
        <div className="text-center text-white px-6">
          <div className="w-12 h-12 bg-white/10 rounded-full flex items-center justify-center mx-auto mb-3">
            <svg
              className="w-6 h-6 text-gray-400"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                strokeWidth={2}
                d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"
              />
            </svg>
          </div>
          <p className="font-medium">Access Required</p>
          <p className="text-sm text-gray-400 mt-1">{error}</p>
        </div>
      </div>
    );
  }

  return (
    <div className="aspect-video rounded-lg overflow-hidden">
      <iframe
        src={embedUrl!}
        title={title}
        className="w-full h-full"
        allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
        allowFullScreen
      />
    </div>
  );
}
