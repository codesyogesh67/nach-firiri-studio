...
export function Modal({
  ...
  return createPortal(
    <div
      className="fixed inset-0 z-[200] flex items-start justify-center p-4 sm:p-10"
    ...
    >
      ...
      <div
        className={cn(
          "relative z-10 w-full mt-4 flex flex-col",
  ...
  return (
    <span
      className={cn(
        ...
        cls[s] ?? "bg-[var(--ag)] text-[var(--gold)] border-[var(--ag)]"
      )}
    >
      {status}
    </span>
...
function StatCard({
  ...
  return (
    <motion.div
      ...
      className="a-card rounded-2xl p-4 sm:p-5"
    >
...
function AdminPage() {
  ...
  const fetchData = async (silent = false) => {
    ...
    try {
      const [b, w] = await Promise.all([
        ...
        fetch(`${SUPABASE_URL}/rest/v1/workshops?order=date.asc`, {
          headers,
        }).then((r) => r.json()),
      ]);
  ...
  const totalRevenue = useMemo(
    () => bookings.reduce((s, b) => s + b.amount_paid, 0),
    [bookings]
  );
  const uniqueAttendees = useMemo(
    () => new Set(bookings.map((b) => b.attendee_email)).size,
    [bookings]
  );
  ...
  const revenueByMonth = useMemo(() => {
    ...
    bookings.forEach((b) => {
      const m = new Date(b.created_at).toLocaleDateString("en-US", {
        month: "short",
      });
  ...
  const filteredBookings = useMemo(() => {
    ...
    if (sFilter !== "all")
      list = list.filter((b) => b.status.toLowerCase() === sFilter);
    list.sort((a, b) => {
  ...
  const handleSave = async () => {
    if (!form.style || !form.date || !form.venue || !form.city) {
      setSaveMsg("Please fill required fields.");
      return;
    }
    setSaving(true);
    setSaveMsg("");
  ...
  return (
    ...
    >
      ...
      >
        ...
        <nav className="flex-1 px-3 py-4 space-y-0.5">
          <p
            ...
            style={{ color: "var(--tm)", letterSpacing: "0.12em" }}
          >
            Main
          </p>
          {navItems.map((item) => (
            <button
              ...
              className={cn(
                ...
                tab === item.id ? "a-nav-active" : "a-nav-item"
              )}
            >
        ...
        >
          <Link
            to="/"
            className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all a-nav-item"
          >
      ...
      <div className="flex flex-col flex-1 min-w-0">
        ...
        >
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className={cn(
                "a-btn-ghost flex items-center gap-1.5 rounded-xl px-2.5 sm:px-3 py-2 text-sm",
            ...
            <button
              ...
              className="flex items-center gap-1.5 rounded-xl px-3 sm:px-4 py-2 text-sm font-semibold transition-all"
              style={{ background: "var(--gold)", color: "#1A1410" }}
            >
        ...
        {/* Mobile tabs */}
        <div
          className="lg:hidden flex border-b overflow-x-auto flex-shrink-0"
          style={{ background: "var(--surface)", borderColor: "var(--border)" }}
        >
          {navItems.map((item) => (
            <button
              ...
              onClick={() => setTab(item.id)}
              className={cn(
                "flex items-center gap-1.5 px-4 py-3 text-sm font-medium whitespace-nowrap border-b-2 transition-all flex-1 justify-center",
                tab === item.id ? "a-mob-active" : "a-mob-inactive"
              )}
            >
            ...
            {tab === "overview" && (
              ...
              >
                ...
                <div className="grid gap-4 sm:gap-5 xl:grid-cols-2">
                  <div className="a-card rounded-2xl p-4 sm:p-5">
                    <p
                      ...
                      <ResponsiveContainer width="100%" height={180}>
                        <BarChart data={bookingsPerWS} barSize={22}>
                          ...
                          <Bar dataKey="bookings" radius={[5, 5, 0, 0]}>
                            {bookingsPerWS.map((_, i) => (
                              <Cell
                                key={i}
                                fill={
                                  i === 0 ? "#C9A96E" : "rgba(201,169,110,0.3)"
                                }
                              />
                            ))}
                          </Bar>
                ...
                <div className="grid gap-4 sm:gap-5 xl:grid-cols-2">
                  <div className="a-card rounded-2xl overflow-hidden">
                    <div className="flex items-center justify-between px-4 sm:px-5 py-4 a-table-head">
                      <p
                    ...
                    {bookings.slice(0, 6).length === 0 ? (
                      ...
                      bookings.slice(0, 6).map((b) => (
                        <div
                          key={b.id}
                          className="flex items-center justify-between px-4 sm:px-5 py-3 a-row"
                        >
                    ...
                    {workshops.slice(0, 6).length === 0 ? (
                      ...
                      workshops.slice(0, 6).map((w) => {
                        ...
                        return (
                          <div
                            key={w.id}
                            className="px-4 sm:px-5 py-3 a-row cursor-pointer"
            ...
            {tab === "workshops" && (
              ...
              >
                <div className="flex items-center justify-between">
                  <p className="text-sm" style={{ color: "var(--tm)" }}>
                    {workshops.length} workshops · {soldOutCount} sold out
                  </p>
                  <div
                  ...
                  >
                    ...
                    <button
                      onClick={() => setWorkshopView("grid")}
                      className={cn(
                        "flex items-center gap-1.5 rounded-lg px-2.5 sm:px-3 py-1.5 text-xs font-medium transition-all",
                        workshopView === "grid" ? "a-view-on" : "a-view-off"
                      )}
                    >
                      ...
                      <table className="w-full">
                        <thead>
                          <tr className="a-table-head">
                            <th
                              className="px-4 sm:px-5 py-3.5 text-left text-xs font-semibold"
                              style={{ color: "var(--tm)" }}
                            >
                              Workshop
                            </th>
                            <th
                              className="px-4 sm:px-5 py-3.5 text-left text-xs font-semibold hidden sm:table-cell"
                              style={{ color: "var(--tm)" }}
                            >
                              Date & Time
                            </th>
                            <th
                              className="px-4 sm:px-5 py-3.5 text-left text-xs font-semibold hidden md:table-cell"
                              style={{ color: "var(--tm)" }}
                            >
                              Venue
                            </th>
                            ...
                            <th
                              className="px-4 sm:px-5 py-3.5 text-left text-xs font-semibold hidden lg:table-cell"
                              style={{ color: "var(--tm)" }}
                            >
                              Revenue
                            </th>
                            ...
                            <th
                              className="px-4 sm:px-5 py-3.5 text-right text-xs font-semibold"
                              style={{ color: "var(--tm)" }}
                            >
                              Action
                            </th>
                        ...
                        <tbody>
                          {workshops.map((w) => {
                            const wBk = bookings.filter(
                              (b) => b.workshop_id === w.id
                            );
                            const rev = wBk.reduce(
                              (s, b) => s + b.amount_paid,
                              0
                            );
                            const pct =
                            ...
                            return (
                              ...
                              >
                                ...
                                <td className="px-4 sm:px-5 py-4">
                                  <div className="flex items-center gap-2">
                                    <div
                                      className="w-12 sm:w-16 h-1.5 rounded-full overflow-hidden flex-shrink-0"
                                      style={{ background: "var(--track)" }}
                                    >
                                      <div
                                        className="h-full rounded-full"
                                ...
                                <td
                                  className="px-4 sm:px-5 py-4 text-right"
                ...
                {workshopView === "grid" && (
                  <div className="grid gap-3 sm:gap-4 sm:grid-cols-2 xl:grid-cols-3">
                    {workshops.map((w) => {
                      ...
                      return (
                        <motion.div
                          ...
                          animate={{ opacity: 1, y: 0 }}
                          className="a-card rounded-2xl p-4 sm:p-5 flex flex-col cursor-pointer"
                          onClick={() => setSelectedWorkshop(w)}
                        >
                          <div className="flex items-start justify-between mb-3">
                          ...
                          >
                            <button
                              onClick={() => setSelectedWorkshop(w)}
                              className="flex-1 flex items-center justify-center gap-1.5 rounded-xl py-2 text-xs font-medium transition-all a-btn-outline"
                            >
                              <Eye className="h-3.5 w-3.5" /> View Guests
                            </button>
                            <button
                              onClick={() => openEdit(w)}
                              className="a-btn-ghost rounded-xl px-3 py-2"
                            >
                              <Edit2 className="h-3.5 w-3.5" />
                            </button>
                          </div>
                    ...
                    <button
                      ...
                      className="a-add-card rounded-2xl p-5 flex flex-col items-center justify-center gap-3 min-h-[180px] sm:min-h-[200px]"
                    >
                      <div
                ...
                <div className="flex flex-col sm:flex-row sm:flex-wrap items-stretch sm:items-center gap-3">
                  ...
                  <div className="flex gap-2">
                    <select
                      ...
                      className="a-input flex-1 rounded-xl px-3 py-2.5 text-sm outline-none"
                    >
                    ...
                    <button
                      ...
                      className="a-btn-ghost flex items-center gap-1.5 rounded-xl px-3 py-2.5 text-sm flex-shrink-0"
                    >
                        ...
                        <tr className="a-table-head">
                          {[
                            ...
                            <th
                              ...
                              className={cn(
                                "px-4 sm:px-5 py-3.5 text-left text-xs font-semibold cursor-pointer hover:opacity-80 select-none transition-opacity",
                                col === "attendee_email" &&
                                ...
                                col === "created_at" && "hidden lg:table-cell"
                              )}
                              style={{ color: "var(--tm)" }}
                            >
                      ...
                      <tbody>
                        {filteredBookings.map((b, i) => (
                          <tr
                            ...
                            className={cn(
                              "a-tr",
                              i === filteredBookings.length - 1 && "!border-0"
                            )}
                          >
                            <td
                              className="px-4 sm:px-5 py-4 text-sm font-medium"
                              style={{ color: "var(--text)" }}
                            >
                              {b.attendee_name}
                            </td>
                            ...
                            <td
                              className="px-4 sm:px-5 py-4 text-sm max-w-[150px] truncate hidden sm:table-cell"
                              style={{ color: "var(--tm)" }}
                            >
                              {b.workshop_name}
                            </td>
                            <td
                              className="px-4 sm:px-5 py-4 text-sm hidden lg:table-cell"
                              style={{ color: "var(--tm)" }}
                            >
                              {b.workshop_date}
                            </td>
                            <td
                              className="px-4 sm:px-5 py-4 text-sm font-semibold"
                              style={{ color: "var(--gold)" }}
                            >
                              {fmt$(b.amount_paid)}
                            </td>
                            ...
                            <td
                              className="px-4 sm:px-5 py-4 text-xs hidden lg:table-cell"
                              style={{ color: "var(--tm)" }}
                            >
                              {fmtDate(b.created_at)}
                            </td>
            ...
            {tab === "analytics" && (
              ...
              >
                <div className="grid grid-cols-2 xl:grid-cols-4 gap-3 sm:gap-4">
                  {[
                    {
                      label: "Avg Revenue / Workshop",
                      value:
                        workshops.length > 0
                          ? `$${(totalRevenue / 100 / workshops.length).toFixed(
                              0
                            )}`
                          : "—",
                    },
                    ...
                    },
                    {
                      ...
                      value:
                        bookings.length > 0
                          ? `$${(totalRevenue / 100 / bookings.length).toFixed(
                              2
                            )}`
                          : "—",
                    },
                ...
                <div className="a-card rounded-2xl overflow-hidden">
                  <div className="px-4 sm:px-5 py-4 a-table-head">
                    <p
                      className="text-sm font-semibold"
                      style={{ color: "var(--text)" }}
                    >
                        ...
                        {workshops.map((w) => {
                          ...
                          return (
                            <tr key={w.id} className="a-tr">
                              ...
                              <td
                                className="px-4 sm:px-5 py-4 text-sm"
                                style={{ color: "var(--tm)" }}
                              >
                                {w.date}
                              </td>
                              ...
                              <td className="px-4 sm:px-5 py-4">
                                <div className="flex items-center gap-2">
                                  <div
                                    className="w-12 sm:w-14 h-1.5 rounded-full overflow-hidden"
                                    style={{ background: "var(--track)" }}
                                  >
                                    <div
                                      className="h-full rounded-full"
